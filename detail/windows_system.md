...
![img](./img/explorer_setting.png)

三点リーダメニュー最下部の `オプション` から各種設定が可能だが、**非推奨**。
理由は、アクセスできないネットワークドライブが表示されているとエクスプローラの起動時にハングするため。
...
`時刻と言語 > 時刻と言語 > 言語と地域` メニュー内の「日本語」の三点リーダオプションから `言語のオプション`
...
設定内で `システム > バージョン情報 > システムの詳細設定` から設定画面を開ける。

![img](./img/system_performance_1.png)

![img](./img/system_performance_2.png)
